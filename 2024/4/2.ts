...
const calculateA = (answer: Answer) => {
  switch (answer.direction) {
    case "diagonalDownRight":
      return [answer.startX + 1, answer.startY + 1];
...
let pairs: Array<Answer[]> = [];

answers.forEach((answer, index) => {
  // console.log(answer);
  const pos = calculateA(answer);
  let match = answers
    .filter((i, idx) => idx !== index && !i.checked)
    .find((i) => JSON.stringify(calculateA(i)) === JSON.stringify(pos));
  if (!match) return;
  answer.checked = true;
  match.checked = true;
  pairs.push([answer, match]);
});
